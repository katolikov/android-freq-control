#!/usr/bin/env python3
"""Generate a per-SoC C++ header by probing an attached Android device, and
re-emit the device registry so the new SoC is selectable at runtime via
`freq_control::Device::k<SOC>`.
...
The script:
    1. Probes the device (SoC name, CPU topology, freq tables, GPU/MIF
       devfreq nodes) and writes include/freq_control/soc/<soc>.h.
    2. Scans include/freq_control/soc/*.h and rewrites
       include/freq_control/device_registry.h so the runtime registry
       lists every compiled-in SoC.

There is no intermediate JSON file and no build-time SoC selection — every
SoC under include/freq_control/soc/ is compiled into the binary.
"""
...
# Where the per-SoC headers and the generated registry live. These paths
# are fixed relative to the project root (the parent of tools/).
_PROJECT_ROOT = Path(__file__).resolve().parent.parent
_SOC_DIR = _PROJECT_ROOT / "include" / "freq_control" / "soc"
_REGISTRY_PATH = _PROJECT_ROOT / "include" / "freq_control" / "device_registry.h"
...
def build_header(spec: dict[str, Any]) -> str:
    ...
    out: list[str] = []
    guard = f"FREQ_CONTROL_SOC_{soc.upper()}_H_"
    out += [
        "// AUTO-GENERATED by tools/gen_device_config.py - DO NOT EDIT BY HAND.",
        "// Regenerate with:",
        f"//   tools/gen_device_config.py --adb-serial <SERIAL>",
        "",
        f"#ifndef {guard}",
        f"#define {guard}",
        "",
...
def _build_registry(soc_names: list[str]) -> str:
    guard = "FREQ_CONTROL_DEVICE_REGISTRY_H_"
    out: list[str] = []
    out += [
        "// AUTO-GENERATED by tools/gen_device_config.py - DO NOT EDIT BY HAND.",
    ...
    for name in soc_names:
        out.append(f'#include "freq_control/soc/{name}.h"')
    out.append("")
    out.append("namespace freq_control {")
    out.append("")
    if not soc_names:
        out.append("// No SoCs are registered. Run tools/gen_device_config.py")
        out.append("// against a device to populate this list.")
        out.append("enum class Device : uint32_t { kNone = 0 };")
        out.append("")
        out.append(
            "inline constexpr DeviceRegistryEntry kDeviceRegistry[1] = "
            "{{Device::kNone, \"\", nullptr}};"
        )
    else:
        out.append("enum class Device : uint32_t {")
        for idx, name in enumerate(soc_names):
            out.append(f"  k{name.upper()} = {idx},")
        out.append("};")
        out.append("")
        out.append("inline constexpr DeviceRegistryEntry kDeviceRegistry[] = {")
        for name in soc_names:
            out.append(
                f'    {{Device::k{name.upper()}, "{name}", &{name}::kDeviceConfig}},'
            )
        out.append("};")
    out.append("")
    out.append("}  // namespace freq_control")
...
def regen_registry() -> None:
    """Re-scan _SOC_DIR and rewrite _REGISTRY_PATH to match."""
    soc_names = sorted(p.stem for p in _SOC_DIR.glob("*.h"))
    _REGISTRY_PATH.parent.mkdir(parents=True, exist_ok=True)
    _REGISTRY_PATH.write_text(_build_registry(soc_names))
    print(
        f"wrote {_REGISTRY_PATH.relative_to(_PROJECT_ROOT)} "
        f"({len(soc_names)} device(s): {', '.join(soc_names) or 'none'})"
    )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "Omit only with --regen-registry to refresh the registry alone.",
    )
    parser.add_argument(
        "--adb",
        default="adb",
    ...
    parser.add_argument(
        "--regen-registry",
        action="store_true",
        help="Skip device probing; just rebuild the registry from the "
        "current contents of include/freq_control/soc/.",
    )
    args = parser.parse_args()

    if not args.regen_registry:
        if not args.adb_serial:
            parser.error("--adb-serial is required (or pass --regen-registry)")
        spec = probe_device(args.adb_serial, args.adb)
        header = build_header(spec)
        out_path = _SOC_DIR / f"{spec['soc']}.h"
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(header)
        print(
            f"wrote {out_path.relative_to(_PROJECT_ROOT)} "
            f"({len(header.splitlines())} lines)"
        )
